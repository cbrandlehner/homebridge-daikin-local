...
getFanStatus: function (callback) {
  ...
  setFanStatus: function (value, callback) {
    ...
    this.sendGetRequest(this.get_control_info, body => {
      ...
      {
        this.log.debug('setFanStatus: Powerstate didnt change, ignore it.');
        if(!(typeof callback === 'undefined')) callback();
        return;
      }
     ...
     let  query = `pow=${targetPOW}&mode=${targetFanMode}&stemp=${responseValues.stemp}&shum=${responseValues.shum}&dt2=${responseValues.dt2}&dh2=${responseValues.dh2}&f_rate=${responseValues.f_rate}&f_dir=${this.swingMode}`;
